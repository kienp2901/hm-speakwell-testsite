...
const AnswerKey = () => {
  ...
  const [historyListening, setHistoryListening] = useState<any>({});
  ...
  const getHistoryExam = async (idHistory: string) => {
    ...
    if (response.status === 200) {
      ...
      setHistoryListening(response?.data?.metadata?.rounds[index]);
  ...
  const getHistoryPart = async (idHistory: string) => {
    const response = await getHistoryDetail(idHistory);
    if (response.status === 200) {
      setHistoryListening(response?.data?.data);
    }
  };

  const handleRedo = async () => {
    router.push(`/practice/listening/${idExam}/${historyListening?.round_id}`);
  };

  useEffect(() => {
    if (pathname.includes('/exam')) {
      getHistoryPart(`${idHistory}`);
    } else {
      getHistoryExam(`${idHistory}`);
    }
  }, [idHistory]);

  useEffect(() => {
    let arrDataResult: any[] = [];
    historyListening?.listQuestionGraded?.map((item: any) => {
    ...
    arrDataResult = arrDataResult.filter((it: any) => {
      return it.answer !== '' && it.answer !== 'undefined';
    });
  ...
  useEffect(() => {
    ...
    historyListening?.listQuestion?.map((item: any) => {
  ...
  return (
    <div className="bg-[#EFF1F4] pt-[62px] sm:pt-16 fixed top-0 left-0 right-0 bottom-0">
      ...
      <div className="h-full overflow-y-auto scroll-smooth pb-10">
        <Scores
          mark={{
            score: Math.round(historyListening?.lms_score * 100) / 100,
            score_max: historyListening?.baikiemtra?.maxMark,
          }}
          correctAnswer={historyListening?.correctAnswer}
          totalQuestion={listDataQuestion?.length}
        />
        <MoreInfo
          className="mx-4 sm:mx-6 lg:mx-20 xl:mx-28 px-6 lg:px-10 xl:px-14"
          metadataAnswer={historyListening}
        />
